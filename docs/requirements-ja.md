...
- **FR-3**: メディアを折りたたんだポストには、**本文の右下**に「Show media」相当のリンクテキストを **1 ポストにつき 1 つ**表示すること。ラベルは `chrome.i18n` でローカライズし(en: "Show media" / ja: 「メディアを表示」)、長文折りたたみ(「さらに表示」)と区別できるようロケールごとに固定とする。ブラウザの UI ロケールに該当メッセージがない場合は `default_locale`(英語)にフォールバックする。本文(`div[data-testid="tweetText"]`)がないメディアのみのポストでは、メディアがあった位置に同リンクを右寄せで表示する。
...
- **FR-15**: ポップアップに「Hide images」「Hide videos」の 2 つのトグル(checkbox)を設けること。ラベルも同様に `chrome.i18n` でローカライズし、UI はブラウザの言語設定に応じて英語/日本語に切り替わる(該当ロケールがなければ英語にフォールバック)。
...
| 本文 | `div[data-testid="tweetText"]` | 「Show media」リンクの挿入位置の基準 |
...
│   └── requirements-ja.md # 本書
├── public/
│   └── _locales/          # chrome.i18n のメッセージバンドル(ビルド出力にそのままコピーされる)
│       ├── en/messages.json
│       └── ja/messages.json
...
├── package.json
├── README.md              # ビルド・インストール手順(英語)
└── README-ja.md           # ビルド・インストール手順(日本語)
```

### manifest 要件

- `name`: "Invisible X Image"
- `description`: `"__MSG_extDescription__"` とし、`_locales/<locale>/messages.json`(en/ja)で解決する
- `default_locale`: `"en"`(ブラウザの UI ロケールに該当バンドルがない場合のフォールバック先)
- `_locales/en/` および `_locales/ja/` に `extDescription` / `showMedia` / `hideImages` / `hideVideos` の各メッセージを用意し、実行時は `browser.i18n.getMessage(...)` で取得する
...
4. x.com のホームタイムラインでメディア付きポストが折りたたまれて本文右下に「Show media」リンクが出て、クリックするとそのポストのメディアが 1 クリックで表示される(手動確認)。プロフィールの「メディア」タブのグリッドは折りたたまれない(手動確認)。
