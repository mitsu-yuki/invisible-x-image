...
1. [Releases ページ](https://github.com/mitsu-yuki/invisible-x-image/releases) から最新の `invisible-x-image-<version>-chrome.zip` をダウンロードし、展開する。
2. Chrome で `chrome://extensions` を開き、右上の「デベロッパーモード」を ON にする。
3. 「パッケージ化されていない拡張機能を読み込む」をクリックし、展開したフォルダを選択する。
4. 拡張機能一覧に「Invisible X Image」が表示されれば完了。
...
## Chrome への読み込み手順（開発ビルドを読み込む場合）

拡張機能を利用したいだけの場合は、上記の[インストール](#インストール)を参照してください。ソースからビルドしたものを読み込む手順は以下の通りです。
